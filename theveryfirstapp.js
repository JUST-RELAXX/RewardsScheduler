...
async function startSession(profileDirs, searchMode = 'edge') {
    ...
    profileDirs.forEach((dir, index) => {
      ...
      `;
            ...
            // Report dimensions after a short delay to allow layout to settle and BlueStacks to spawn
            setTimeout(() => {
                ...
                const trackBounds = () => {
                    ...
                    // Send pure logical coordinates (Windows DPI virtualization handles scaling)
                    const newX = Math.round(updatedRect.left);
                    const newY = Math.round(updatedRect.top);
                    const newW = Math.round(updatedRect.width);
                    const newH = Math.round(updatedRect.height);
            ...
            }, 5000); // 5s delay gives enough time for emulator window to spawn before overlay.exe tries finding it
