...
class Overlay
{
    ...
    static void Main(string[] args)
    {
        ...
        // Wait up to 30 seconds for the window to exist
        for (int i = 0; i < 300; i++)
        {
        ...
        // Set Electron window as the TRUE parent. This ruthlessly clips BlueStacks inside it.
        SetParent(bsHwnd, electronHwnd);

        // Force frame recalculation so it stops clipping the content
        SetWindowPos(bsHwnd, IntPtr.Zero, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED | SWP_NOACTIVATE);

        Console.WriteLine("Syncing position...");
        ...
        // Run continuous loop to sync position
        ...
        while (true)
        {
            ...
            {
                ShowWindow(bsHwnd, SW_SHOW);
                wasMinimized = false;
            }

            // Convert absolute screen coordinates to relative client coordinates of the Electron window
            POINT pt = new POINT { X = offsetX, Y = offsetY };
            ScreenToClient(electronHwnd, ref pt);

            // Update BlueStacks window position and size relative to parent client area
            SetWindowPos(bsHwnd, IntPtr.Zero, pt.X, pt.Y, width, height, SWP_NOZORDER | SWP_NOACTIVATE);

            Thread.Sleep(16); // Sync at roughly 60 FPS
        }

        Console.WriteLine("Overlay exit.");
    }
    ...
    static IntPtr FindVisibleBlueStacksWindow(string expectedTitle)
    {
        ...
        EnumWindows((hWnd, lParam) =>
        {
            if (IsWindowVisible(hWnd))
            {
                ...
                {
                    return true; // Skip splash screens or borderless windows
                }
